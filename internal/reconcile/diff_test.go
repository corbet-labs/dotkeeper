...
func TestBuildDesired(t *testing.T) {
	...
	t.Run("machine name is set from machine config; peers come from state", func(t *testing.T) {
		...
		if d.MachineName != "laptop" {
			t.Errorf("expected MachineName=laptop, got %q", d.MachineName)
		}
		if len(d.Peers) != 2 {
		...
		repos := map[string]*config.RepoConfigV2{
			"/home/user/dotfiles": {
				Sync: config.RepoSyncConfig{
					SyncthingFolderID: "dk-dotfiles",
					ShareWith:         []string{"laptop", "server"},
					Ignore:            []string{"*.log"},
				},
			},
		}
...
func TestBuildDesired_PeersFromState(t *testing.T) {
	...
	if d.Peers[0].Name != "laptop" || d.Peers[0].DeviceID != "ABC123" {
		t.Errorf("peer[0]: expected {laptop, ABC123}, got %+v", d.Peers[0])
	}
