...
`
	...
	if desired.Peers[0].Name != "laptop" || desired.Peers[0].DeviceID != "ABC123-XYZ" {
		t.Errorf("peer[0]: expected {laptop, ABC123-XYZ}, got %+v", desired.Peers[0])
	}
...
func TestObservedProvider_NilClient(t *testing.T) {
	...
	provider := newObservedProvider(nil, statePath, nil, nil, nil, nil)
...
func TestObservedProvider_SyncthingUnreachable(t *testing.T) {
	...
	provider := newObservedProvider(q, statePath, nil, nil, nil, nil)
